...
class Fast_dLLM_QwenForCausalLM(Fast_dLLM_QwenPreTrainedModel, GenerationMixin):
    ...
    @staticmethod
    def _resolve_block_mask_num(block_mask_num, bd_size: int) -> int:
        if block_mask_num is None:
            raise ValueError("block_mask_num is required for trajectory-based masking.")
        if isinstance(block_mask_num, torch.Tensor):
    ...
    @staticmethod
    def _build_trajectory_mask_indices(
        ...
        """
        每个 block：取块内 trajectory 最大值为 trajectory_end（k = block_mask_num）。
        - trajectory_end > 0：mask 满足 trajectory > trajectory_end - k 且 labels != -100。
        - trajectory_end == -1：mask 当前 block 末尾 k 个可训练 token。
        含可训练 token 的 block 要求 trajectory_end != 0。
        """
        ...
        k = Fast_dLLM_QwenForCausalLM._resolve_block_mask_num(block_mask_num, bd_size)
        ...
        mask_indices = torch.where(use_fallback, fallback_mask, traj_mask)
        return mask_indices.to(device=device)
    ...
    def forward(
        ...
        trajectory: Optional[torch.LongTensor] = None,
        block_mask_num: Optional[Union[int, float, torch.Tensor]] = None,
        **kwargs
    ) -> CausalLMOutputWithPastAndBlockCache:
        if self.training:
            if trajectory is None:
                raise ValueError(
                    "trajectory is required in training forward for trajectory-based masking."
                )
            if block_mask_num is None:
                raise ValueError(
                    "block_mask_num is required in training forward for trajectory-based masking."
                )
            ...
            block_flat = input_ids.reshape(
                input_ids.shape[0] * input_ids.shape[1] // self.model.bd_size,
                self.model.bd_size,
            )
            mask_indices = self._build_trajectory_mask_indices(
                labels,
                trajectory,
                block_mask_num,
                self.model.bd_size,
            )
            x_t = torch.where(mask_indices, mask_id, block_flat).reshape(labels.shape)
            noisy_input_ids[labels != -100] = x_t[labels != -100]
            mask = (noisy_input_ids != mask_id)
            labels[mask] = -100
            input_ids = torch.cat([noisy_input_ids, block_flat.reshape(labels.shape)], dim=1)

            complementary_noisy_input_ids = original_input_ids.clone()
            complementary_labels = original_labels.clone()

            complementary_block_flat = original_input_ids.reshape(
                original_input_ids.shape[0] * original_input_ids.shape[1] // self.model.bd_size,
                self.model.bd_size,
            )

            complementary_mask_indices = ~mask_indices
            complementary_x_t = torch.where(
                complementary_mask_indices, mask_id, complementary_block_flat
            ...
            complementary_input_ids = torch.cat(
                [complementary_noisy_input_ids, complementary_block_flat.reshape(complementary_labels.shape)],
                dim=1,
            )
