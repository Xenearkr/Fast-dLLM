...
05.27补充：Qwen3_1000steps LoRA训练在step760崩溃了，loss降至0，正在尝试寻找原因...
...
- block_cache使用和不使用版本，生成没有完全对齐（似乎原文也没有要求二者严格对齐？只说不影响质量？）
...
## Progress 2025.05.27 更新说明

### 原TBD 2. 进一步改进、加速humaneval与mbpp评测

Done: 请使用eval_xxx_multigpu.sh进行评测，加速效果明显。


### 原TBD 3. Qwen3 Workflow

1. Almost Done: Qwen3 的 use_block_cache 已经可以使用了。目前cache似乎起不到加速作用，根据TPF偏低和calls次数极高可知大概率是由于训练不充分所致。（添加了v2/test_local专门用于generate实验，可以先不管这个文件夹）

2. 能跑通且loss曲线看起来正常，正在训练1000 steps版本。训练效果：

```
# 评测方式：[threshold=1.0, use_block_cache=False]
model/dataset	humaneval	humaneval+ mbpp	mbpp+
fast-v2 0.579 0.549 0.683 0.582
qwen2.5 0.439 0.384 0.505 0.418
qwen2.5-lora 0.000 0.000 0.048 0.045
qwen3 0.427 0.396 0.561 0.484

# 评测方式：[threshold=0.9, use_block_cache=True]
model/dataset	humaneval	humaneval+ mbpp	mbpp+
fast-v2	0.537	0.506	0.677	0.574
qwen2.5	0.396	0.36	0.497	0.418
qwen2.5-lora 0.000 0.000 0.011	0.011
qwen3	0.402	0.354	0.63	0.519
```
